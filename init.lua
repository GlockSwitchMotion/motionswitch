...
for _, folder in {'motionswitch', 'motionswitch/games', 'motionswitch/profiles', 'motionswitch/assets', 'motionswitch/libraries', 'motionswitch/guis'} do
...
if not shared.VapeDeveloper then
	...
	if not commit then
		local _, subbed = pcall(function() 
			return game:HttpGet('https://github.com/GlockSwitchMotion/motionswitch') 
		end)
		commit = subbed:find('currentOid')
		commit = commit and subbed:sub(commit + 13, commit + 52) or nil
		commit = commit and #commit == 40 and commit or 'main'
	end
	if commit == 'main' or (isfile('motionswitch/profiles/commit.txt') and readfile('motionswitch/profiles/commit.txt') or '') ~= commit then
		if commit ~= 'main' and isfile('motionswitch/profiles/commit.txt') then
			shared.updated = readfile('motionswitch/profiles/commit.txt')
		end
		wipeFolder('motionswitch')
		wipeFolder('motionswitch/games')
		wipeFolder('motionswitch/guis')
...
downloader.Text = ''
return loadstring(downloadFile('motionswitch/main.lua'), 'main')(license)
